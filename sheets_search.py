...
with col_left:
    ...
    if sheets_data:
        title_col, del_col = st.columns([3, 1])
        with title_col:
            st.markdown('<div class="section-title" style="margin-top:2rem">📂 등록된 시트 목록</div>', unsafe_allow_html=True)
        with del_col:
            st.write("")
            st.write("")
            if st.button("🗑️ 전체 삭제", use_container_width=True, key="clear_btn"):
                st.session_state["confirm_clear"] = True

        if st.session_state.get("confirm_clear"):
            st.warning("⚠️ 전체 삭제하면 복구할 수 없어요!")
            confirm = st.checkbox("네, 전부 삭제할게요", key="confirm_check")
            if confirm:
                clear_all()
