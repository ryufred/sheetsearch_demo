...
def update_row(row_index, entry):
    ws = get_worksheet()
    ws.update(f"A{row_index+2}:E{row_index+2}",
              [[entry["id"], entry["name"], entry["url"], entry["tags"], entry["desc"]]])
...
with col_left:
    ...
    new_tags_raw = st.text_input("태그 (쉼표 구분)", placeholder="예) 경영지원팀, 영상팀, 본사1팀")
        ...
        for s in paged_list:
            ...
            with st.expander(f"📄 {s['name']}"):
                ...
                if not is_editing:
                    ...
                    edit_name = st.text_input("시트 이름", value=s["name"], key=f"edit_name_{i}")
                    edit_url  = st.text_input("구글 시트 URL", value=s["url"], key=f"edit_url_{i}")
                    edit_tags = st.text_input("태그 (쉼표 구분)", value=s.get("tags",""), key=f"edit_tags_{i}")
                    edit_desc = st.text_input("간단한 설명", value=s.get("desc",""), key=f"edit_desc_{i}")
                    scol1, scol2 = st.columns(2)
                    with scol1:
                        if st.button("💾 저장", key=f"save_{i}", use_container_width=True):
                            if not edit_name.strip() or not edit_url.strip():
                                st.error("시트 이름과 URL은 비워둘 수 없어요.")
                            else:
                                update_row(i, {"id": s.get("id", datetime.now().isoformat()),
                                                "name": edit_name.strip(), "url": edit_url.strip(),
                                                "tags": edit_tags.strip(), "desc": edit_desc.strip()})
                                st.session_state[edit_key] = False
                                st.success("수정 완료!")
                                st.rerun()
                    with scol2:
                        if st.button("✖️ 취소", key=f"cancel_{i}", use_container_width=True):
                            st.session_state[edit_key] = False
                            st.rerun()
...
with col_right:
    ...
    with tab4:
        if not sheets_data:
            ...
            if selected and st.button("👥 공유 목록 불러오기"):
                ...
                if perms and "error" in perms[0]:
                    ...
                    rows_html = ""
                    for p in perms:
                        email = p.get("emailAddress", "링크 공유 (익명)")
